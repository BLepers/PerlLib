...
sub run_cmd {
   ...
   #print "Executing command ($self -> $cmd) on host $host\n" if $debug;
   #print "Executing command ($self -> $cmd) on host $host\n";

   #syswrite(STDERR,"Executing command ($self -> $cmd) on host $host\n");

   #my $ssh = Net::OpenSSH->new($host, master_opts => [-o => "StrictHostKeyChecking=no"]);
   #my ($in, $out, $err, $pid) = $ssh->open3($cmd);
   ...
   my $status = sshopen3($host, *WRITER, *READER, *READER, $cmd);
   if(!$status) {
...
sub start_profilers {
   ...
   my $async_thread = threads::async { 
      ...
      if ( $self->{oprofile} ) {
         my $op_cmd = "opcontrol --deinit; opcontrol --start-daemon; opcontrol --reset; echo `date` > /tmp/oprof.dump; opcontrol --start;";
         my $thr = threads->create( 'ClusterSSHNode::run_cmd', $self, $op_cmd);
         $thr->detach();
      }
...
sub stop_profilers {
   ...
   if ( $self->{oprofile} ) {
      my $op_cmd = "opcontrol --stop; opcontrol --dump; opreport -l >> /tmp/oprof.dump; echo `date` >> /tmp/oprof.dump;";
      $self->run_cmd( $op_cmd );
		$cmd = "scp $SSH_OPTS -q $self:/tmp/oprof.dump oprof.$self.dump;";
      system $cmd;
   }
}
   ...
   # First start profilers if needed
	if ( $opt->{profile} ) {
		for my $n (values %$nodes) {
         $n->start_profilers($self->{sar}, $self->{oprofile});
      }
	}
