...
sub sum_1_sum_3_div_sum_0_sum_2_per_core {
   ...
   my $event_3 = $self->_scripted_value_to_event(3, $info);
   ...
   for my $core (sort {$a <=> $b} keys %{$self->{miniprof}->{raw}}) {
      ...
      my ($avg3, $sum3, $count3) = File::MiniProf::_miniprof_get_average_and_sum($self->{miniprof}->{raw}->{$core}, $event_3 );
      ...
      if(($sum0 + $sum2) != 0) {
         $info->{results}->{$core} = ($sum1+$sum3)/($sum0 + $sum2);
      }
      ...
      if($opt->{gnuplot}) {
         if(!defined($opt->{gnuplot_max_cpu}) || $core < $opt->{gnuplot_max_cpu}) {
            ...
            for (my $i = 0; $i < scalar (@{$self->{miniprof}->{raw}->{$core}->{$event_0}->{val}}); $i++) {
               ...
               my $val_3 = $self->{miniprof}->{raw}->{$core}->{$event_3}->{val}->[$i];
               my $avg = ($val_0 + $val_2)?(($val_1 + $val_3)/($val_0 + $val_2)):0;
               push(@vals, $avg);
            }
   ...
   if($glob_sum_0 + $glob_sum_2){
      $info->{results}->{ALL} = ($glob_sum_1 + $glob_sum_3) / ($glob_sum_0 + $glob_sum_2);
   }
   else {
